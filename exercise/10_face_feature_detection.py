...
def findMaxContour(contours):   # max alanı bul ve max indisi
    max_i = 0  # max alan indis
    max_area = 0 # max alan

    for i in range(len(contours)):  #alanları birbirleriyle karşılaştırıp max olanı al
...
while 1:
    ret,frame = cap.read()
    frame = cv2.flip(frame,1) # görüntüyü kameradan aldığımız için flip işlemi yapılır
    roi = frame[50:250,200:400] # frameler üzerinde alan belirleme

    cv2.rectangle(frame,(200,50),(400,250),(0,0,255),0) # belirlenen alana kare çizimi
    ...
    mask = cv2.dilate(mask,kernel=kernel,iterations=1)  # dilate,nesen dınırlarını genişletir
    ...
    if len(contours)>0:
        try:
            ...
            extLeft = tuple(c[c[:,:,0].argmin()][0]) # konturları dolaşıp en kucuk x'i bulacak
